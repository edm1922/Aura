...
// Particles component
function Particles({ traits, count = 50 }: TraitProps & { count?: number }) {
...
// Main Orb component
function Orb({ traits, animated = true }: TraitProps & { animated?: boolean }) {
  const { openness = 3, conscientiousness = 3, extraversion = 3, agreeableness = 3, neuroticism = 3 } = traits || {}
...
function Scene({ traits, animated, interactive }: TraitProps & { animated?: boolean; interactive?: boolean }) {
  return (
    <>
...
export default function ThreeComponents({
  ...
  interactive = true,
  size = 200,
}: TraitProps & { animated?: boolean; interactive?: boolean; size?: number }) {
  return (
    <Canvas
